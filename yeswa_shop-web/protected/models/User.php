...
class User extends \app\components\TActiveRecord implements \yii\web\IdentityInterface
{
    ...
    const LOCATION_TRACKING_ON = 1;

    const LOCATION_TRACKING_OFF = 0;
    ...
    public function attributeLabels()
    {
        return [
            ...
            'contact_no_1' => Yii::t('app', 'Contact No'),
     ...
     * return $this->hasOne ( VendorLocation::className (), [
     * 'created_by_id' => 'id'
     * ] );
     * }
     * public function getVendorAddress() {
     * return $this->hasOne ( VendorAddress::className (), [
     * 'created_by_id' => 'id'
     * ] );
     * }
     */
     ...
     * return $this->hasOne ( VendorProfile::className (), [
     * 'created_by_id' => 'id'
     * ] );
     * }
     */
    ...
    public function sendOrderMailtoUser()
    {
        $sub = 'New Order Placed Successfully';
        $from = $this->email;
        EmailQueue::sendEmailToAdmins([
            'from' => $from,
            'subject' => $sub,
    ...
    public function SendMail()
    {
        ...
        $mail = $mailer->compose();
        // $user = User::findOne('212');
        /*
         * $html = $this->renderPartial('..\..\mail\sendPassword.php', [
         * 'user' => $user,
         * 'password' => 'dsfdssf'
         * ]);
         */
        $mail->setTo('admin@toxsl.in')
            ->setFrom("user@toxsl.com")
            ->setHtmlBody("order placed")
    ...
    public function beforeDelete()
    {
        ...
        VendorAddress::deleteRelatedAll([
            'created_by_id' => $this->id
        ]);
        VendorLocation::deleteRelatedAll([
            'created_by_id' => $this->id
        ]);
        VendorProfile::deleteRelatedAll([
            'created_by_id' => $this->id
        ]);
        ...
        Brand::deleteRelatedAll([
            'created_by_id' => $this->id
        ]);
        
        ContactUs::deleteRelatedAll([
            'created_by_id' => $this->id
        ]);
    ...
    public function asJson($with_relations = false)
    {
        ...
        if ($this->role_id == User::ROLE_VENDOR) {
            $json['first_name'] = isset($this->vendorProfile) ? $this->vendorProfile->first_name : "";
            $json['last_name'] = isset($this->vendorProfile) ? $this->vendorProfile->last_name : "";
        } else {
        ...
        $json['contact_no_1'] = $this->contact_no_1;
        ...
        if (isset($this->profile_file))
            $json['profile_file'] = $this->getImageUrl(true);
        $json['tos'] = $this->tos;
        $json['role_id'] = $this->role_id;
        $json['is_customer'] = $this->is_customer;
        $json['is_vendor'] = $this->is_vendor;
        ...
        $json['location_tracking'] = $this->location_tracking;
        if ($with_relations) {
            
            // address
            $list = $this->getVendorAddress()->one();
            
            $json['vendorAddress'] = $list->asJson();
            
            // / profile
            $list = $this->getVendorProfile()->one();
            $json['vendorProfile'] = $list->asJson();
            
            // location
            
            // / profile
            $list = $this->getVendorLocation()->one();
            
            // $relationData = [ ];
            
            $json['vendorLocation'] = $list->asJson();
        }
    ...
    public static function isVendor()
    {
        ...
        return ($user->isActive() && $user->role_id == User::ROLE_VENDOR || self::isAdmin());
    }
    ...
    public function exportFields()
    {
        return [
            ...
            'full_name',
            'contact_no',
            'whats_app_no' => function ($model) {
                return $model->getRelatedUserData(VendorProfile::className(), 'whats_app_no');
            },
            'civil_id' => function ($model) {
                return $model->getRelatedUserData(VendorProfile::className(), 'civil_id');
            },
            'shopname' => function ($model) {
                return $model->getRelatedUserData(VendorProfile::className(), 'shopname');
            },
            'location' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'location');
            },
            'city' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'city');
            },
            'area' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'area');
            },
            'city' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'city');
            },
            'block' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'block');
            },
            'street' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'street');
            },
            'house' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'house');
            },
            'apartment' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'apartment');
            },
            'office' => function ($model) {
                return $model->getRelatedUserData(VendorAddress::className(), 'office');
            }
     ...
     */
    public function getRelatedUserData($model, $attribute)
    {
        $data = $model::find()->where([
            'created_by_id' => $this->id
        ])->one();
        return ! empty($data) ? $data->$attribute : 'Not Set';
    }

    public function getImageUrl($thumbnail = false, $default = 'default.png')
    {
        $params = [
        ...
        $params['file'] = isset($this->profile_file) ? $this->profile_file : null;
        if ($thumbnail)
            $params['thumbnail'] = is_numeric($thumbnail) ? $thumbnail : 150;
        $params['defaultImg'] = $default;
