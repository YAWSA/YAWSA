<?php

/**
 * This is the model class for table "tbl_product".
 *
 * @property integer $id
 * @property string $title
 * @property string $description
 * @property integer $brand_id
 * @property integer $category_id
 * @property integer $state_id
 * @property integer $type_id
 * @property string $created_on
 * @property string $updated_on
 * @property integer $created_by_id
 
 * === Related data ===
 * @property User $createdBy
 * @property ProductVariant[] $productVariants
 */
namespace app\models;

use Yii;
use yii\helpers\ArrayHelper;

class Product extends \app\components\TActiveRecord
{

    public $not_active = 0;

    public $active = 1;

    public function __toString()
    {
        return (string) $this->title;
    }

    const STATE_INACTIVE = 0;

    const STATE_ACTIVE = 1;

    const STATE_DELETED = 2;

    const IS_FAVOURITE_YES = 1;

    const IS_FAVOURITE_NO = 0;
    

    public static function getStateOptions()
    {
        return [
            self::STATE_INACTIVE => "New",
            self::STATE_ACTIVE => "Active",
            self::STATE_DELETED => "Archived"
        ];
    }

    public function getState()
    {
        $list = self::getStateOptions();
        return isset($list[$this->state_id]) ? $list[$this->state_id] : 'Not Defined';
    }

    public function getStateBadge()
    {
        $list = [
            self::STATE_INACTIVE => "primary",
            self::STATE_ACTIVE => "success",
            self::STATE_DELETED => "danger"
        ];
        return isset($list[$this->state_id]) ? \yii\helpers\Html::tag('span', $this->getState(), [
            'class' => 'label label-' . $list[$this->state_id]
        ]) : 'Not Defined';
    }

    public static function getCategoryOptions()
    {
        return ArrayHelper::Map(Category::findActive()->all(), 'id', 'title');
    }

    public static function getBrandOptions()
    {
        return ArrayHelper::Map(Brand::findActive()->all(), 'id', 'title');
    }

    public static function getTypeOptions()
    {
        return [
            "TYPE1",
            "TYPE2",
            "TYPE3"
        ];
        // return ArrayHelper::Map ( Type::findActive ()->all (), 'id', 'title' );
    }

    public function getType()
    {
        $list = self::getTypeOptions();
        return isset($list[$this->type_id]) ? $list[$this->type_id] : 'Not Defined';
    }

    public function beforeValidate()
    {
        if ($this->isNewRecord) {
            if (! isset($this->created_on))
                $this->created_on = date('Y-m-d H:i:s');
            if (! isset($this->updated_on))
                $this->updated_on = date('Y-m-d H:i:s');
            if (! isset($this->created_by_id))
                $this->created_by_id = Yii::$app->user->id;
        } else {
            $this->updated_on = date('Y-m-d H:i:s');
        }
        return parent::beforeValidate();
    }

    /**
     * @inheritdoc
     */
    public static function tableName()
    {
        return '{{%product}}';
    }

    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [
                [
                    'title',
                    'description',
                    'brand_id',
                    'category_id',
                    'created_on',
                    'updated_on',
                    'created_by_id'
                ],
                'required'
            ],
            [
                [
                    'state_id',
                    'type_id',
                    'created_by_id'
                ],
                'integer'
            ],
            [
                [
                    'created_on',
                    'updated_on'
                ],
                'safe'
            ],
            [
                [
                    'title'
                ],
                'string',
                'max' => 256
            ],
            [
                [
                    'description'
                ],
                'string',
                'max' => 1000
            ],
            [
                [
                    'created_by_id'
                ],
                'exist',
                'skipOnError' => true,
                'targetClass' => User::className(),
                'targetAttribute' => [
                    'created_by_id' => 'id'
                ]
            ],
            [
                [
                    'title',
                    'description'
                ],
                'trim'
            ],
            [
                [
                    'state_id'
                ],
                'in',
                'range' => array_keys(self::getStateOptions())
            ],
            [
                [
                    'type_id'
                ],
                'in',
                'range' => array_keys(self::getTypeOptions())
            ]
        ];
    }

    /**
     * @inheritdoc
     */
    public function attributeLabels()
    {
        return [
            'id' => Yii::t('app', 'ID'),
            'title' => Yii::t('app', 'Title'),
            'description' => Yii::t('app', 'Description'),
            'category_id' => Yii::t('app', 'Category'),
            'brand_id' => Yii::t('app', 'Brand'),
            'state_id' => Yii::t('app', 'State'),
            'type_id' => Yii::t('app', 'Type'),
            'created_on' => Yii::t('app', 'Created On'),
            'updated_on' => Yii::t('app', 'Updated On'),
            'created_by_id' => Yii::t('app', 'Created By')
        ];
    }

    /**
     *
     * @return \yii\db\ActiveQuery
     */
    public function getCreatedBy()
    {
        return $this->hasOne(User::className(), [
            'id' => 'created_by_id'
        ]);
    }

    public function getCategory()
    {
        return $this->hasOne(Category::className(), [
            'id' => 'category_id'
        ]);
    }

    public function getBrand()
    {
        return $this->hasOne(Brand::className(), [
            'id' => 'brand_id'
        ]);
    }
    
    public function getSale()
    {
        return $this->hasOne(Sale::className(), [
            'model_id' => 'id'
        ]);
    }

    public function getFiles()
    {
        return $this->hasMany(File::className(), [
            'model_id' => 'id'
        ])->andWhere([
            'model_type' => self::className()
        ]);
    }

    /**
     *
     * @return \yii\db\ActiveQuery
     */
    public function getProductVariants()
    {
        return $this->hasMany(ProductVariant::className(), [
            'product_id' => 'id'
        ]);
    }

    public static function getHasManyRelations()
    {
        $relations = [];
        $relations['Files'] = [
            'files',
            'File',
            'id',
            'model_id'
        ];
        $relations['ProductVariants'] = [
            'productVariants',
            'ProductVariant',
            'id',
            'product_id'
        ];
        return $relations;
    }

    public static function getHasOneRelations()
    {
        $relations = [];
        $relations['created_by_id'] = [
            'createdBy',
            'User',
            'id'
        ];
        
        $relations['brand_id'] = [
            'brand',
            'Brand',
            'id'
        ];
        $relations['category_id'] = [
            'category',
            'Category',
            'id'
        ];
        return $relations;
    }

    public function beforeDelete()
    {
        ProductVariant::deleteRelatedAll([
            'product_id' => $this->id
        ]);
        
        File::deleteRelatedAll([
            'model_id' => $this->id,
            'model_type' => get_class($this)
        ]);
        return parent::beforeDelete();
    }

    public function getFileList()
    {
        $models = File::find()->where([
            'model_id' => $this->id,
            'model_type' => get_class(new self())
        ])->all();
        $data = [];
        if (! empty($models)) {
            foreach ($models as $model) {
                $file['id'] = $model->id;
                $file['image_file'] = $model->getImageUrl(true);
                $data[] = $file;
            }
        }
        
        return $data;
    }

    public function getFavouriteList()
    {
        $models = ProductFavourite::find()->where([
            'product_id' => $this->id,
            'created_by_id' => 148
        ])->all();
        $data = [];
        if (! empty($models)) {
            foreach ($models as $model) {
                
                $file['id'] = $model->id;
                
                // $file['is_favourite'] = 1
                $data[] = $file;
            }
        }
        
        return $data;
    }

    public function getcheckIsFavourite()
    {
        $model = ProductFavourite::find()->where([
            'created_by_id' => Yii::$app->user->id,
            'product_id' => $this->id
        ])->one();
        if ($model) {
            return self::IS_FAVOURITE_YES;
        }
        return self::IS_FAVOURITE_NO;
    }

    
    
    public function asJson($with_relations = false)
    {
        $json = [];
        
        $json['id'] = $this->id;
        $json['title'] = $this->title;
        $json['description'] = $this->description;
        $json['category_id'] = $this->category_id;
        $json['brand_id'] = $this->brand_id;
        $json['state_id'] = $this->state_id;
        $json['type_id'] = $this->type_id;
        
        $json['created_on'] = $this->created_on;
        $json['created_by_id'] = $this->created_by_id;
        $json['favourite_list'] = $this->getFavouriteList();
        $json['is_favourite'] = $this->getcheckIsFavourite();
        $json['file_list'] = $this->getFileList();
        //$json['sale'] = !empty($this->sale) ? $this->sale : [];
        
        if ($with_relations) {
            // brand
            $list = $this->brand;
            
            if (is_array($list)) {
                $relationData = [];
                foreach ($list as $item) {
                    $relationData[] = $item->asJson();
                }
                $json['brand'] = $relationData;
            } else {
                $json['Brand'] = $list;
            }
            // category
            $list = $this->category;
            
            if (is_array($list)) {
                $relationData = [];
                foreach ($list as $item) {
                    $relationData[] = $item->asJson();
                }
                $json['category'] = $relationData;
            } else {
                $json['Category'] = $list;
            }
            // createdBy
            $list = $this->createdBy;
            
            if (is_array($list)) {
                $relationData = [];
                foreach ($list as $item) {
                    $relationData[] = $item->asJson();
                }
                $json['createdBy'] = $relationData;
            } else {
                $json['CreatedBy'] = $list;
            }
            // product-favourite
            
            // $list = $this->FavouriteList;
            
            // if (is_array($list)) {
            // $relationData = [];
            // foreach ($list as $item) {
            // $relationData[] = $item->asJson();
            // }
            // $json['productFavourite'] = $relationData;
            // } else {
            // $json['productFavourite'] = $list;
            // }
            // productVariants
            
            if (! User::isGuest()) {
                if (\Yii::$app->user->identity->role_id == User::ROLE_VENDOR) {
                    $list = $this->productVariants;
                    
                    if (is_array($list)) {
                        $relationData = [];
                        foreach ($list as $item) {
                            $relationData[] = $item->asColorJson(true);
                        }
                        $json['productVariants'] = $this->getProductVarients();
                    } else {
                        $json['ProductVariants'] = $list;
                    }
                } else {
                    
                    $json['ProductVariants'] = $this->getProductVarients();
                }
            } else {
                $json['ProductVariants'] = $this->getProductVarients();
            }
            // $json['ProductVariants'] = $this->getProductVarients();
        }
        return $json;
    }

    public function loadFaker()
    {
        if (YII_ENV == 'prod')
            return;
        
        $this->title = \Faker\Factory::create()->text(10);
        $this->description = \Faker\Factory::create()->text(10);
    }

    public function getProductVarients()
    {
        $data = [];
        $varients = ProductVariant::find()->groupBy('color_id')
            ->where([
            'product_id' => $this->id
        ])
            ->all();
        if (! empty($varients)) {
            foreach ($varients as $varient) {
                $model = ProductVariant::find()->where([
                    'product_id' => $this->id,
                    'color_id' => $varient->color_id
                ])->one();
                if (! empty($model)) {
                    $data[] = $model->asColorJson(true);
                }
            }
        }
        
        return $data;
    }
}
