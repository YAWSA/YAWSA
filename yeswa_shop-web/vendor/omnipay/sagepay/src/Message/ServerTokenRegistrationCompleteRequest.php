...
class ServerTokenRegistrationCompleteRequest extends AbstractRequest
{
    ...
    public function getSignature()
    {
        ...
        $vpsTxId = str_replace(array(
            '{',
            '}'
        ), '', $reference['VPSTxId']);
        
        // Re-create the VPSSignature
        $signature_string = $vpsTxId . $reference['VendorTxCode'] . $this->httpRequest->request->get('Status') . $this->getVendor() . $this->httpRequest->request->get('Token') . $reference['SecurityKey'];
