...
abstract class SecureXMLAbstractRequest extends AbstractRequest
{
    ...
    {
        $httpResponse = $this->httpClient->post($this->getEndpoint(), null, $data->asXML())
            ->send();
    ...
    protected function getBasePaymentXMLWithCard()
    {
        ...
        $card->addChild('cardNumber', $this->getCard()
            ->getNumber());
        $card->addChild('cvv', $this->getCard()
            ->getCvv());
        $card->addChild('expiryDate', $this->getCard()
            ->getExpiryDate('m/y'));
