...
abstract class PayeezyAbstractRequest extends \Omnipay\Common\Message\AbstractRequest
{
    ...
    /**
     * API version to use.
     * See the note about the hashing requirements for v12 or higher.
     */
    ...
    {
        return sprintf("%s\n%s\n%s\n%s\n%s", self::METHOD_POST, self::CONTENT_TYPE, $contentDigest, $gge4Date, $uri);
    }
    ...
    {
        return sprintf('GGE4_API %s:%s', $this->getKeyId(), base64_encode(hash_hmac("sha1", $hashString, $this->getHmac(), true)));
    }
