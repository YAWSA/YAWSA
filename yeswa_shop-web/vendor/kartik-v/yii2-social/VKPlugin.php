...
class VKPlugin extends Widget
{
    ...
    public function init()
    {
        ...
        $initRequired = ! in_array($this->type, [
            self::POST,
            self::COMMUNITY,
            self::SHARE,
            self::SUBSCRIBE
        ...
        if (! isset($this->noscript)) {
            $this->noscript = Yii::t('kvsocial', 'Please enable JavaScript on your browser to view the VKontakte {pluginName} plugin correctly on this site.', [
                'pluginName' => Yii::t('kvsocial', str_replace('vk_', '', $this->type))
            ]);
        }
