...
/**
 * Dependent Dropdown action that can be used to generate the dependent option values via ajax response.
 * A typical
...
class DepDropAction extends Action
{
    ...
    public function run()
    {
        ...
        if (($selected = $request->post($this->parentParam)) && is_array($selected) && ! empty($selected[0])) {
            ...
            $id = $selected[0];
            return [
                'output' => $this->getOutput($id, $params),
                'selected' => $this->getSelected($id, $params)
            ];
        }
        return [
            'output' => '',
