...
class Infinite_Scroll_Admin
{
    ...
    function __construct(&$parent)
    {
        ...
        // upload helpers
        add_filter('get_media_item_args', array(
            &$this,
            'send_to_editor'
        ), 10, 1);
    }
    ...
    {
        add_options_page(__('Infinite Scroll Options', 'infinite-scroll'), __('Infinite Scroll', 'infinite-scroll'), 'manage_options', 'infinite_scroll_options', array(
            &$this,
            'options'
        ));
    }
    ...
    function admin_enqueue()
    {
        ...
        wp_enqueue_script($this->parent->slug, plugins_url($file, $this->parent->file), array(
            'jquery',
            'media-upload',
            'thickbox'
        ), $this->parent->version, true);
        wp_enqueue_style('thickbox');
        
        wp_localize_script($this->parent->slug, $this->parent->slug_, array(
            'confirm' => __('Are you sure you want to delete the preset "%s"?', 'infinite-scroll')
        ));
    }
     ...
     * 
     * @param string $field
     *            the field to load the editor for
     */
    function editor($field)
    {
        ...
        if (function_exists('wp_editor'))
            wp_editor($this->parent->options->loading[$field], "infinite_scroll[loading][{$field}]", array(
                'media_buttons' => false,
                'textarea_rows' => 5,
                'teeny' => true
            ));
