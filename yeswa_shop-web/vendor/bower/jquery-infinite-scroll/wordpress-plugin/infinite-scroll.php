...
class Infinite_Scroll
{
    ...
    public $name = 'Infinite Scroll';
 // Human-readable name of plugin
    public $slug = 'infinite-scroll';
 // plugin slug, generally base filename and in url on wordpress.org
    public $slug_ = 'infinite_scroll';
 // slug with underscores (PHP/JS safe)
    public $prefix = 'infinite_scroll_';
 // prefix to append to all options, API calls, etc. w/ trailing underscore
    public $file = null;
    ...
    function __construct()
    {
        ...
        add_action('wp_footer', array(
            &$this,
            'footer'
        ), 100); // low priority will load after i18n and script loads
    ...
    function enqueue_js()
    {
        ...
        wp_enqueue_script($this->slug, plugins_url($file, __FILE__), array(
            'jquery'
        ), $this->version, true);
        ...
        wp_enqueue_script($this->slug . "-behavior", plugins_url($src, __FILE__), array(
            "jquery",
            $this->slug
        ), $this->version, true);
    }
    ...
    function upgrade($from, $to)
    {
        if ($from < "2.5") {
            ...
            if (isset($new["loading"]['img']) && ! strstr($new["loading"]["img"], "/img/ajax-loader.gif"))
                $new["loading"]['img'] = str_replace("/ajax-loader.gif", "/img/ajax-loader.gif", $new["loading"]['img']);
