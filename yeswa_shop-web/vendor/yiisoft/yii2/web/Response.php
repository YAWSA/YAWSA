...
class Response extends \yii\base\Response
{
     ...
     * @param bool $inline
     *            whether the browser should open the file within the browser window. Defaults to false,
     *            meaning a download dialog will pop up.
    ...
    {
        $rangeHeader = Yii::$app->getRequest()
            ->getHeaders()
            ->get('Range', '-');
        if ($rangeHeader === '-') {
            return [
    ...
    {
        $fallbackName = str_replace('"', '\\"', str_replace([
            '%',
            '/',
            '\\'
        ], '_', Inflector::transliterate($attachmentName, Inflector::TRANSLITERATE_LOOSE)));
        $utfName = rawurlencode(str_replace([
            '%',
            '/',
            '\\'
        ], '', $attachmentName));
     ...
     *            the HTTP status code. Defaults to 302.
        ...
        if ($checkAjax) {
            if (Yii::$app->getRequest()->getIsAjax()) {
                if (Yii::$app->getRequest()
                    ->getHeaders()
                    ->get('X-Ie-Redirect-Compatibility') !== null && $statusCode === 302) {
    ...
    {
        return $this->redirect(Yii::$app->getRequest()
            ->getUrl() . $anchor);
    }
