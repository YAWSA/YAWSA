...
SQL;
        $sequenceName = $this->db->createCommand($sequenceNameSql, [
            ':tableName' => $tableName
        ])->queryScalar();
    ...
    public function getLastInsertID($sequenceName = '')
    {
        if ($this->db->isActive) {
            ...
            return $this->db->useMaster(function (Connection $db) use ($sequenceName) {
                return $db->createCommand("SELECT {$sequenceName}.CURRVAL FROM DUAL")
                    ->queryScalar();
            });
        } else {
...
SQL;
        ...
        $constraints = ArrayHelper::index($constraints, null, [
            'type',
            'name'
        ]);
        $result = [
