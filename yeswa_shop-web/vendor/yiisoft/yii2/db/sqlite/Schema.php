...
class Schema extends \yii\db\Schema
{
    ...
    {
        $foreignKeys = $this->db->createCommand('PRAGMA FOREIGN_KEY_LIST (' . $this->quoteValue($tableName) . ')')
            ->queryAll();
    ...
    public function findUniqueIndexes($table)
    {
        ...
        foreach ($indexes as $index) {
            ...
            $indexInfo = $this->db->createCommand('PRAGMA index_info(' . $this->quoteValue($index['name']) . ')')
                ->queryAll();
    ...
    {
        $indexes = $this->db->createCommand('PRAGMA INDEX_LIST (' . $this->quoteValue($tableName) . ')')
            ->queryAll();
        ...
        if (! empty($indexes) && ! isset($indexes[0]['origin'])) {
             ...
             */
            $tableColumns = $this->db->createCommand('PRAGMA TABLE_INFO (' . $this->quoteValue($tableName) . ')')
                ->queryAll();
        ...
        foreach ($indexes as $index) {
            $columns = $this->db->createCommand('PRAGMA INDEX_INFO (' . $this->quoteValue($index['name']) . ')')
                ->queryAll();
