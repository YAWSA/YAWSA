...
class QueryBuilder extends \yii\db\QueryBuilder
{
    ...
    public function resetSequence($tableName, $value = null)
    {
        ...
        if ($table !== null && $table->sequenceName !== null) {
            ...
            if ($value === null) {
                ...
                $value = (int) $this->db->createCommand("SELECT MAX(`$key`) FROM " . $this->db->schema->quoteTableName($tableName))
                    ->queryScalar() + 1;
            } else {
                $value = (int) $value;
            }
    ...
    {
        $row = $this->db->createCommand('SHOW CREATE TABLE ' . $this->db->quoteTableName($table))
            ->queryOne();
        if ($row === false) {
