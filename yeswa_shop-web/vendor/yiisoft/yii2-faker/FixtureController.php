...
class FixtureController extends \yii\console\controllers\FixtureController
{
    ...
    /**
     * Returns array containing fixtures templates file names.
     * You can specify what files to find
    ...
    private function findTemplatesFiles(array $templatesNames = [])
    {
        ...
        if ($findAll) {
            $files = FileHelper::findFiles(Yii::getAlias($this->templatePath), [
                'only' => [
                    '*.php'
                ]
            ]);
    ...
    /**
     * Returns Faker generator instance.
