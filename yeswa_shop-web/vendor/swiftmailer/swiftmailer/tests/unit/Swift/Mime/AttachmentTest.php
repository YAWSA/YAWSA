...
class Swift_Mime_AttachmentTest extends Swift_Mime_AbstractMimeEntityTest
{
    ...
    {
        $disposition = $this->createHeader('Content-Disposition', 'attachment', array(), false);
    ...
    {
        $cType = $this->createHeader('Content-Type', '', array(), false);
    ...
    public function testFilenameIsReturnedFromHeader()
    {
         ...
         * -- RFC 2183, 2.3.
         */
        $disposition = $this->createHeader('Content-Disposition', 'attachment', array(
            'filename' => 'foo.txt'
        ));
        $attachment = $this->createAttachment($this->createHeaderSet(array(
            'Content-Disposition' => $disposition
    ...
    public function XtestContentTypeCanBeLookedUpFromCommonListIfNotProvided()
    {
        ...
        $attachment = $this->createAttachment($headers, $this->createEncoder(), $this->createCache(), array(
            'zip' => 'application/zip',
            'txt' => 'text/plain'
        ));
        $attachment->setFile($file);
