...
class AbstractLexerTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->lexer->setInput('1');
        $this->assertTrue($this->lexer->isNextAny(array(
            self::T_COMMA,
            self::T_INTEGER
        )));
        $this->assertFalse($this->lexer->isNextAny(array(
            self::T_COMMA,
            self::T_STRING
        )));
    }
    ...
    public function testPeek()
    {
        ...
        $this->assertValue(2, $this->lexer->peek()
            ->get());
        
        $this->assertValue(1, $this->lexer->next);
        $this->assertValue(3, $this->lexer->peek()
            ->get());
    ...
    protected function setUp()
    {
        ...
        $this->lexer->expects($this->any())
            ...
            if (preg_match('/^[0-9]+$/', $value)) {
                return array(
                    AbstractLexerTest::T_INTEGER,
                    (integer) $value
                );
            }
            
            return array(
                AbstractLexerTest::T_UNKNOWN,
                $value
            );
        }));
    }
