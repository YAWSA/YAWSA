...
abstract class AbstractParser
{
     ...
     *            parsing context (allows to produce better error messages)
     *            
     * @return mixed
     */
    ...
    protected function matchAny(array $types)
    {
        if (! $this->lexer->isNextAny($types)) {
            $this->syntaxError('any of ' . implode(' or ', array_map(array(
                $this->lexer,
                'getName'
            ), $types)));
        }
     ...
     *
     * @param string $expectedDesc
     *            A human understandable explanation what was expected
     * @param array $actualToken
     *            The token that was found. If not given, next token will be assumed.
     */
