 ...
 // Clear first, then add a value.
    ...
    function __construct()
    {
        ...
        $this->DirectoryString = array(
            ...
            'children' => array(
                'teletexString' => array(
                    'type' => ASN1::TYPE_TELETEX_STRING
                ),
                'printableString' => array(
                    'type' => ASN1::TYPE_PRINTABLE_STRING
                ),
                'universalString' => array(
                    'type' => ASN1::TYPE_UNIVERSAL_STRING
                ),
                'utf8String' => array(
    ...
    function getDN($format = self::DN_ARRAY, $dn = null)
    {
        ...
        foreach ($dn['rdnSequence'] as $field) {
            ...
            $output .= $desc . '=' . $value;
            $result[$desc] = isset($result[$desc]) ? array_merge((array) $dn[$prop], array(
                $value
            )) : $value;
            $start = false;
        }
    ...
    function setAttribute($id, $value, $disposition = self::ATTR_ALL)
    {
        ...
        switch (true) {
            ...
            default:
                $attributes[] = array(
                    'type' => $id,
                    'value' => $disposition == self::ATTR_ALL ? $value : array(
                        $value
                    )
                );
                break;
