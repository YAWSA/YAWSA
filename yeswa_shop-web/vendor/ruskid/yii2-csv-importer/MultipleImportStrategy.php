...
/**
 * Will import from CSV.
 * This will batch insert the rows, no validation is performed.
...
class MultipleImportStrategy extends BaseImportStrategy implements ImportInterface
{
     ...
     * 
     * @param array $data
     *            CSV data passed by reference to save memory.
    ...
    public function import(&$data)
    {
        ...
        foreach ($chunks as $chunk) { // Execute multiple queries
            $countInserts += \Yii::$app->db->createCommand()
                ->batchInsert($this->tableName, $attributes, $chunk)
                ->execute();
        }
        return $countInserts;
    }
    ...
    /**
     * Will filter values per unique parameters.
     * Config array can receive 1+ unique parameters.
     * 
     * @param array $values
     */
