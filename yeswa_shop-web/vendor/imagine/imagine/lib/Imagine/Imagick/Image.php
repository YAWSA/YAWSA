...
final class Image implements ImageInterface
{
    ...
    public function thumbnail(BoxInterface $size, $mode = ImageInterface::THUMBNAIL_INSET)
    {
        ...
        try {
            if ($mode === ImageInterface::THUMBNAIL_INSET) {
                $thumbnail->imagick->thumbnailImage($size->getWidth(), $size->getHeight(), true);
            } elseif ($mode === ImageInterface::THUMBNAIL_OUTBOUND) {
                $thumbnail->imagick->cropThumbnailImage($size->getWidth(), $size->getHeight());
            }
        } catch (\ImagickException $e) {
    ...
    public function histogram()
    {
        ...
        return array_map(function (\ImagickPixel $pixel) {
            $info = $pixel->getColor();
