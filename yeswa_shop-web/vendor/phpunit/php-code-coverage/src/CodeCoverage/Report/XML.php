...
class PHP_CodeCoverage_Report_XML
{
    ...
    private function initTargetDirectory($dir)
    {
        if (file_exists($dir)) {
            if (! is_dir($dir)) {
                throw new PHP_CodeCoverage_RuntimeException("'$dir' exists but is not a directory.");
            }
            
            if (! is_writable($dir)) {
                throw new PHP_CodeCoverage_RuntimeException("'$dir' exists but is not writable.");
            }
        } elseif (! @mkdir($dir, 0777, true)) {
            throw new PHP_CodeCoverage_RuntimeException("'$dir' could not be created.");
        }
    }
    ...
    private function processDirectory(PHP_CodeCoverage_Report_Node_Directory $directory, PHP_CodeCoverage_Report_XML_Node $context)
    {
        ...
        foreach ($directory as $node) {
            ...
            throw new PHP_CodeCoverage_RuntimeException('Unknown node type for XML report');
        }
    }
