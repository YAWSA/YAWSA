...
EOF;
    ...
    public function afterSuite()
    {
        ...
        $indexHTML = (new Template($this->indexTemplate))->place('seed', $this->seed)
            ->place('records', $links)
            ->produce();
    ...
    public function before(TestEvent $e)
    {
        ...
        $testName = str_replace([
            ...
            '',
            ''
        ], TestCase::getTestSignature($e->getTest()));
    ...
    public function persist(TestEvent $e)
    {
        ...
        foreach ($this->slides as $i => $step) {
            $indicatorHtml .= (new Template($this->indicatorTemplate))->place('step', (int) $i)
                ->place('isActive', (int) $i ? '' : 'class="active"')
                ->produce();
            
            $slideHtml .= (new Template($this->slidesTemplate))->place('image', $i)
        ...
        $html = (new Template($this->template))->place('indicators', $indicatorHtml)
            ->place('slides', $slideHtml)
            ->place('feature', ucfirst($e->getTest()
            ->getFeature()))
