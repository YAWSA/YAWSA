...
class RoboFile extends \Robo\Tasks
{
    ...
    /**
     * creates codecept.phar with Guzzle 5.3 and Symfony 2.8
     * 
     * @throws Exception
     */
    ...
    private function packPhar($pharFileName)
    {
        ...
        $code = $this->taskExec('php ' . $pharFileName)
            ->run()
            ->getExitCode();
        if ($code !== 0) {
    ...
    /**
     * generates modules reference from source files
     */
    ...
    public function buildDocsModules()
    {
        ...
        $this->say("Modules");
        $modules = Finder::create()->files()
            ->name('*.php')
            ->in(__DIR__ . '/src/Codeception/Module');
    ...
    public function buildDocsCommands()
    {
        ...
        $commands = Finder::create()->files()
            ->name('*.php')
            ->depth(0)
            ->in(__DIR__ . '/src/Codeception/Command');
        ...
        $commandGenerator->prepend("# Console Commands\n")
    ...
    public function buildDocsExtensions()
    {
        ...
        $extensions = Finder::create()->files()
            ->sortByName()
            ->name('*.php')
            ->in(__DIR__ . '/ext');
        ...
        $extGenerator->prepend("# Official Extensions\n")
            ->processClassSignature(function ($r, $text) {
            return "## " . $r->getName();
        })
            ->filterMethods(function (ReflectionMethod $r) {
    ...
    /**
     * publishes generated phar to codeception.com
     */
    public function publishPhar()
    {
        ...
        $releases = array_reverse(iterator_to_array(Finder::create()->depth(0)
            ->directories()
            ->sort($sortByVersion)
            ->in('releases')));
        $branch = null;
    ...
    /**
     * creates a new version tag and pushes to github
     * 
     * @param null $branch
     * @param array $opt
     */
    ...
    protected function processChangelog()
    {
        ...
        // issue
        $changelog = preg_replace('~#(\d+)~', '<a href="https://github.com/Codeception/Codeception/issues/$1">#$1</a>', $changelog);
    ...
    /**
     * cleans all log and temp directories
     */
    ...
    public function buildActors()
    {
        ...
        $this->taskExec($build)
            ->args('-c tests/data/claypit')
            ->run();
        $this->taskExec($build)
            ->args('-c tests/data/included')
            ->run();
        $this->taskExec($build)
            ->args('-c tests/data/included/jazz')
            ->run();
        $this->taskExec($build)
            ->args('-c tests/data/included/shire')
            ->run();
        $this->taskExec($build)
            ->args('-c tests/data/included/jazz')
            ->run();
