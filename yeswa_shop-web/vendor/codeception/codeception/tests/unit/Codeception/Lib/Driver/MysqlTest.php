...
class MysqlTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertNotEmpty($this->mysql->getDbh()
            ->query("SHOW TABLES")
            ->fetchAll());
        $this->mysql->cleanup();
        $this->assertEmpty($this->mysql->getDbh()
            ->query("SHOW TABLES")
            ->fetchAll());
    }
    ...
    public function testGetSingleColumnPrimaryKey()
    {
        $this->assertEquals([
            'id'
        ], $this->mysql->getPrimaryKey('order'));
    }
    ...
    public function testGetCompositePrimaryKey()
    {
        $this->assertEquals([
            'group_id',
            'id'
        ], $this->mysql->getPrimaryKey('composite_pk'));
    }
    ...
    {
        $this->setExpectedException('\Exception', 'getPrimaryColumn method does not support composite primary keys, use getPrimaryKey instead');
    ...
    {
        $res = $this->mysql->executeQuery("select `id` from `users` where `is_active` = ?", [
            false
        ]);
