...
class PostgresTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertNotEmpty($this->postgres->getDbh()
            ->query("SELECT * FROM pg_tables where schemaname = 'public'")
            ->fetchAll());
        $this->postgres->cleanup();
        $this->assertEmpty($this->postgres->getDbh()
            ->query("SELECT * FROM pg_tables where schemaname = 'public'")
            ->fetchAll());
    }
    ...
    public function testGetSingleColumnPrimaryKey()
    {
        $this->assertEquals([
            'id'
        ], $this->postgres->getPrimaryKey('order'));
    }
    ...
    public function testGetCompositePrimaryKey()
    {
        $this->assertEquals([
            'group_id',
            'id'
        ], $this->postgres->getPrimaryKey('composite_pk'));
    }
