...
class SqliteTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $this->assertGreaterThan(0, count(self::$sqlite->getDbh()
            ->query('SELECT name FROM sqlite_master WHERE type = "table";')
            ->fetchAll()));
        self::$sqlite->cleanup();
        $this->assertEmpty(self::$sqlite->getDbh()
            ->query('SELECT name FROM sqlite_master WHERE type = "table";')
            ->fetchAll());
    }
    ...
    public function testGetSingleColumnPrimaryKey()
    {
        $this->assertEquals([
            'id'
        ], self::$sqlite->getPrimaryKey('order'));
    }
    ...
    public function testGetCompositePrimaryKey()
    {
        $this->assertEquals([
            'group_id',
            'id'
        ], self::$sqlite->getPrimaryKey('composite_pk'));
    }
