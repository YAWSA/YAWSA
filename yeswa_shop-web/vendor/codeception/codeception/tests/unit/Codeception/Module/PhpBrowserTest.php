...
class PhpBrowserTest extends TestsForBrowsers
{
    ...
    {
        return Stub::makeEmpty('\Codeception\TestCase\Cept', array(
    ...
    public function testAjax()
    {
        ...
        $this->module->sendAjaxPostRequest('/form/complex', array(
            'show' => 'author'
        ));
        $this->assertNotNull(data::get('ajax'));
        $post = data::get('form');
    ...
    public function testHeadersBySetHeader()
    {
        ...
        $this->module->amOnPage('/');
        $this->assertTrue($this->getLastRequest()
            ->hasHeader('xxx'));
    }
    ...
    public function testDeleteHeaders()
    {
        ...
        $this->module->amOnPage('/');
        $this->assertFalse($this->getLastRequest()
            ->hasHeader('xxx'));
    }
    ...
    public function testDoubleSlash()
    {
        ...
        $formUrl = $this->module->client->getHistory()
            ->current()
            ->getUri();
    ...
    {
        $this->setExpectedException('Codeception\Exception\ModuleException');
        $response = $this->module->_request('POST', '/form/try', [
            'user' => 'davert'
        ]);
        $data = data::get('form');
    ...
    {
        $this->module->_loadPage('POST', '/form/try', [
            'user' => 'davert'
        ]);
        $data = data::get('form');
        $this->assertEquals('davert', $data['user']);
