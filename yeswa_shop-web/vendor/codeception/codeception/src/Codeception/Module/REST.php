...
EOF;
    ...
    {
        return [
            'Codeception\Lib\InnerBrowser' => $this->dependencyMessage
        ];
    }
    ...
    public function _inject(InnerBrowser $connection)
    {
        ...
        if ($this->connectionModule instanceof PhpBrowser) {
            if (! $this->connectionModule->_getConfig('url')) {
                $this->connectionModule->_setConfig([
                    'url' => $this->config['url']
                ]);
            }
        }
    ...
    {
        if ($value !== null) {
            $this->assertEquals($value, $this->getRunningClient()
                ->getInternalResponse()
                ->getHeader($name));
            return;
        }
        $this->assertNotNull($this->getRunningClient()
            ->getInternalResponse()
            ->getHeader($name));
    }
    ...
    {
        if ($value !== null) {
            $this->assertNotEquals($value, $this->getRunningClient()
                ->getInternalResponse()
                ->getHeader($name));
            return;
        }
        $this->assertNull($this->getRunningClient()
            ->getInternalResponse()
            ->getHeader($name));
    }
    ...
    {
        $headers = $this->getRunningClient()
            ->getInternalResponse()
            ->getHeader($name, false);
    ...
    {
        return $this->getRunningClient()
            ->getInternalResponse()
            ->getHeader($name, $first);
    }
    ...
    protected function execute($method, $url, $parameters = [], $files = [])
    {
        ...
        if (count($this->client->getInternalRequest()->getCookies())) {
            $this->debugSection('Cookies', $this->client->getInternalRequest()
                ->getCookies());
        }
        $this->debugSection("Headers", $this->client->getInternalResponse()
            ->getHeaders());
        $this->debugSection("Status", $this->client->getInternalResponse()
            ->getStatus());
    }

    protected function encodeApplicationJson($method, $parameters)
    {
        if ($method !== 'GET' && array_key_exists('Content-Type', $this->headers) && ($this->headers['Content-Type'] === 'application/json' || preg_match('!^application/.+\+json$!', $this->headers['Content-Type']))) {
    ...
    public function seeResponseIsJson()
    {
        ...
        \PHPUnit_Framework_Assert::assertEquals(JSON_ERROR_NONE, $errorCode, sprintf("Invalid json: %s. System message: %s.", $this->connectionModule->_getResponseContent(), json_last_error_msg()));
    }
    ...
    public function seeResponseContainsJson($json = [])
    {
        ...
        \PHPUnit_Framework_Assert::assertTrue($jsonResponseArray->containsArray($json), "Response JSON does not contain the provided JSON\n" . "- <info>" . var_export($json, true) . "</info>\n" . "+ " . var_export($jsonResponseArray->toArray(), true));
    }
    ...
    public function seeResponseJsonMatchesXpath($xpath)
    {
        ...
        $this->assertGreaterThan(0, (new JsonArray($response))->filterByXPath($xpath)->length, "Received JSON did not match the XPath `$xpath`.\nJson Response: \n" . $response);
    }
    ...
    {
        $response = $this->connectionModule->_getResponseContent();
        $this->assertNotEmpty((new JsonArray($response))->filterByJsonPath($jsonPath), "Received JSON did not match the JsonPath provided\n" . $response);
    }
    ...
    {
        $response = $this->connectionModule->_getResponseContent();
        $this->assertEmpty((new JsonArray($response))->filterByJsonPath($jsonPath), "Received JSON did (but should not) match the JsonPath provided\n" . $response);
    }
    ...
    {
        $jsonResponseArray = new JsonArray($this->connectionModule->_getResponseContent());
        $this->assertFalse($jsonResponseArray->containsArray($json), "Response JSON contains provided JSON\n" . "- <info>" . var_export($json, true) . "</info>\n" . "+ " . var_export($jsonResponseArray->toArray(), true));
    }
     ...
     *            structure
     * @param null $jsonPath
     *            optionally set specific path to structure with JsonPath
     * @version 2.1.3
     */
    ...
    {
        $this->assertEquals($code, $this->getRunningClient()
            ->getInternalResponse()
            ->getStatus());
    }
    ...
    {
        $this->assertNotEquals($code, $this->getRunningClient()
            ->getInternalResponse()
            ->getStatus());
    }
