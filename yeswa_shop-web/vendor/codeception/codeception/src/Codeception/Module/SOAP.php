...
EOF;
    ...
    public function seeSoapResponseEquals($xml)
    {
        ...
        $this->assertEquals($this->getXmlResponse()
            ->C14N(), $xml->C14N());
    }
    ...
    public function seeSoapResponseIncludes($xml)
    {
        ...
        $this->assertContains($xml, $this->getXmlResponse()
            ->C14N(), "found in XML Response");
    }
    ...
    {
        $xml = $this->canonicalize($xml);
        $this->assertNotContains($xml, $this->getXmlResponse()
            ->C14N(), "found in XML Response");
    }
    ...
    public function seeSoapResponseContainsStructure($xml)
    {
        ...
        $this->assertTrue((bool) $this->getXmlStructure()
            ->matchXmlStructure($xml), "this structure is in response");
    }
    ...
    public function dontSeeSoapResponseContainsStructure($xml)
    {
        $xml = SoapUtils::toXml($xml);
        $this->debugSection("Structure", $xml->saveXML());
        $this->assertFalse((bool) $this->getXmlStructure()
            ->matchXmlStructure($xml), "this structure is in response");
    }
    ...
    {
        $this->assertTrue($this->getXmlStructure()
            ->matchesXpath($xpath));
    }
    ...
    {
        $this->assertFalse($this->getXmlStructure()
            ->matchesXpath($xpath));
    }
    ...
    {
        $this->assertEquals($code, $this->client->getInternalResponse()
            ->getStatus(), "soap response code matches expected");
    }
    ...
    {
        $this->getClient()->request('POST', $this->config['endpoint'], [], [], [
