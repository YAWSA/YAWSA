 ...
 * ## Methods
 */
class WebDriver extends CodeceptionModule implements WebInterface, RemoteInterface, MultiSessionInterface, SessionSnapshot, ScreenshotSaver, PageSourceSaver, ElementLocator
{
    ...
    public function _initialize()
    {
        ...
        try {
            $this->webDriver = RemoteWebDriver::create($this->wd_host, $this->capabilities, $this->connectionTimeoutInMs, $this->requestTimeoutInMs, $this->httpProxy, $this->httpProxyPort);
        ...
        $this->webDriver->manage()
            ->timeouts()
            ->implicitlyWait($this->config['wait']);
    ...
    public function _failed(TestCase $test, $fail)
    {
        ...
        $filename = str_replace([
            ...
            '',
            ''
        ], TestCase::getTestSignature($test)) . '.fail';
    ...
    public function _getUrl()
    {
        if (! isset($this->config['url'])) {
            throw new ModuleConfigException(__CLASS__, "Module connection failure. The URL for client can't bre retrieved");
        }
    ...
    {
        $cookies = $this->filterCookies($this->webDriver->manage()
            ->getCookies(), $params);
        $cookies = array_map(function ($c) {
            return $c['name'];
        }, $cookies);
        $this->debugSection('Cookies', json_encode($this->webDriver->manage()
            ->getCookies()));
    ...
    {
        $this->assertThat($this->webDriver->getPageSource(), new PageConstraint($text, $this->_getCurrentUri()), '');
    }
    ...
    {
        $this->assertThatItsNot($this->webDriver->getPageSource(), new PageConstraint($text, $this->_getCurrentUri()), '');
    }
    ...
    protected function findClickable($page, $link)
    {
        ...
        // wide
        $xpath = Locator::combine(".//a[./@href][((contains(normalize-space(string(.)), $locator)) or .//img[contains(./@alt, $locator)])]", ".//input[./@type = 'submit' or ./@type = 'image' or ./@type = 'button'][contains(./@value, $locator)]", ".//input[./@type = 'image'][contains(./@alt, $locator)]", ".//button[contains(normalize-space(string(.)), $locator)]", ".//input[./@type = 'submit' or ./@type = 'image' or ./@type = 'button'][./@name = $locator]", ".//button[./@name = $locator]");
    ...
    protected function findFields($selector)
    {
        ...
        $xpath = Locator::combine(
            ...
            ".//*[self::input | self::textarea | self::select][not(./@type = 'submit' or ./@type = 'image' or ./@type = 'hidden')][(((./@name = $locator) or ./@id = //label[contains(normalize-space(string(.)), $locator)]/@for) or ./@placeholder = $locator)]", ".//label[contains(normalize-space(string(.)), $locator)]//.//*[self::input | self::textarea | self::select][not(./@type = 'submit' or ./@type = 'image' or ./@type = 'hidden')]")
    ...
    {
        $this->assertTrue($this->findField($checkbox)
            ->isSelected());
    }

    public function dontSeeCheckboxIsChecked($checkbox)
    {
        $this->assertFalse($this->findField($checkbox)
            ->isSelected());
    }
    ...
    public function seeOptionIsSelected($selector, $optionText)
    {
        ...
        if ($el->getTagName() !== 'select') {
            ...
            $this->assertNotEmpty(array_filter($els, function ($e) {
    ...
    public function seeInPopup($text)
    {
        ...
        $this->assertContains($text, $this->webDriver->switchTo()
            ->alert()
            ->getText());
    }
     ...
     * values.
     * Give the form fields values as an array. Note that hidden fields
     * can't be accessed.
     *
    ...
    {
        $this->assertThat(htmlspecialchars_decode($this->getVisibleText()), new PageConstraint($needle, $this->_getCurrentUri()), $message);
    }

    protected function assertPageNotContains($needle, $message = '')
    {
        $this->assertThatItsNot(htmlspecialchars_decode($this->getVisibleText()), new PageConstraint($needle, $this->_getCurrentUri()), $message);
    }
    ...
    {
        $els = $this->match($this->webDriver, $selector);
        $nodes = array_filter($els, function (WebDriverElement $el) {
            return $el->isDisplayed();
        });
        return $nodes;
    }
    ...
    public function saveSessionSnapshot($name)
    {
        ...
        foreach ($this->webDriver->manage()->getCookies() as $cookie) {
            if (in_array(trim($cookie['name']), [
                LocalServer::COVERAGE_COOKIE,
                LocalServer::COVERAGE_COOKIE
            ])) {
                continue;
            }
