...
EOF;
    ...
    {
        return [
            'Codeception\Module\PhpBrowser' => $this->dependencyMessage
        ];
    }
     ...
     *
     * @param bool $renew
     *            true if the test user should be recreated
     */
    public function haveFacebookTestUserAccount($renew = false)
    {
        ...
        if (! array_key_exists('id', $this->testUser)) {
            $this->testUser = $this->facebook->createTestUser($this->config['test_user']['name'], $this->config['test_user']['permissions']);
        }
    }
     ...
     *            Place identifier to be verified against user published posts
     */
     ...
     *            published post to be verified against the actual post on facebook
     */
