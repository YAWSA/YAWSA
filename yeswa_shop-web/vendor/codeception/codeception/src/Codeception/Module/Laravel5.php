...
class Laravel5 extends Framework implements ActiveRecord, PartedModule
{
    ...
    protected function checkBootstrapFileExists()
    {
        ...
        if (! file_exists($bootstrapFile)) {
            throw new ModuleConfigException($this, "Laravel bootstrap file not found in $bootstrapFile.\n" . "Please provide a valid path to it using 'bootstrap' config param. ");
        }
    }
     ...
     *            The authentication driver for Laravel <= 5.1.*, guard name for Laravel >= 5.2
     * @return void
     */
    ...
    protected function modelFactory($model, $name, $times)
    {
        if (! function_exists('factory')) {
            throw new ModuleException($this, 'The factory() method does not exist. ' . 'This functionality relies on Laravel model factories, which were introduced in Laravel 5.1.');
        }
    ...
    {
        $internalDomains = [
            $this->getApplicationDomainRegex()
        ];
