...
class Db extends CodeceptionModule implements DbInterface
{
    ...
    public function seeNumRecords($expectedNumber, $table, array $criteria = [])
    {
        ...
        $this->assertEquals($expectedNumber, $actualNumber, sprintf('The number of found rows (%d) does not match expected number %d for criteria %s in table %s', $actualNumber, $expectedNumber, json_encode($criteria), $table));
    }
