...
class Yii2 extends Framework implements ActiveRecord, PartedModule
{
     ...
     * ```
     *
     * @param $page string|array
     *            parameter for \yii\web\UrlManager::createUrl()
     */
    ...
    private function getDomainRegex($template)
    {
        ...
        if (strpos($template, '<') !== false) {
            $template = preg_replace_callback('/<(?:\w+):?([^>]+)?>/u', function ($matches) use (&$parameters) {
    ...
    {
        $domains = [
            $this->getDomainRegex(Yii::$app->urlManager->hostInfo)
        ];
