...
EOF;
    ...
    public function _depends()
    {
        ...
        return [
            'Codeception\Lib\Interfaces\DoctrineProvider' => $this->dependencyMessage
        ];
    }
    ...
    /**
     * Adds entity to repository and flushes.
     * You can redefine it's properties with the second parameter.
     *
     * Example:
     *
     * ``` php
    ...
    public function haveFakeRepository($classname, $methods = [])
    {
        ...
        $mock = Stub::make($customRepositoryClassName, array_merge([
        ...
        if ($reflectedEm->hasProperty('repositories')) {
            $property = $reflectedEm->getProperty('repositories');
            $property->setAccessible(true);
            $property->setValue($em, array_merge($property->getValue($em), [
                $classname => $mock
            ]));
        } else {
    ...
    protected function proceedSeeInRepository($entity, $params = [])
    {
        ...
        return [
            'True',
            (count($res) > 0),
            "$entity with " . json_encode($params)
        ];
    }
