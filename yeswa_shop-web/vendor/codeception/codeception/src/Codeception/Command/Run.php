...
class Run extends Command
{
    ...
    public function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $userOptions = array_merge($userOptions, $this->booleanOptions($input, [
            'xml',
            'html',
            'json',
            'tap',
        ...
        $userOptions['interactive'] = ! $input->hasParameterOption([
            '--no-interaction',
            '-n'
        ]);
        ...
        foreach ($tokens as $token) {
            ...
            if (strpos($token, '-') === 0) {
                ...
                $options[] = $this->getDefinition()
                    ->getOptionForShortcut($shortOption)
                    ->getName();
            }
        }
        return $options;
    ...
    protected function booleanOptions(InputInterface $input, $options = [])
    {
        ...
        foreach ($options as $option) {
            if (strpos($request, "--$option")) {
                $values[$option] = $input->hasParameterOption($option) ? $input->getParameterOption($option) : $input->getOption($option);
            } else {
                $values[$option] = false;
            }
        }
