...
class Guzzle6 extends Client
{
    ...
    protected function createResponse(Psr7Response $response)
    {
        ...
        if ($status < 300 || $status >= 400) {
            ...
            $matchesMeta = preg_match('/\<meta[^\>]+http-equiv="refresh" content="\s*(\d*)\s*;\s*url=(.*?)"/i', $body, $matches);
            ...
            if ((! empty($matches)) && (empty($matches[1]) || $matches[1] < $this->refreshMaxInterval)) {
                ...
                $currentUri = new Psr7Uri($this->getHistory()
                    ->current()
                    ->getUri());
    ...
    protected function doRequest($request)
    {
        ...
        $options['cookies'] = $this->extractCookies($guzzleRequest->getUri()
            ->getHost());
