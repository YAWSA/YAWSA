...
class Guzzle extends Client
{
    ...
    /**
     * Sets the request header to the passed value.
     * The header will be
    ...
    protected function createResponse(Response $response)
    {
        ...
        if ($status < 300 || $status >= 400) {
            ...
            $matchesMeta = preg_match('/\<meta[^\>]+http-equiv="refresh" content="\s*(\d*)\s*;\s*url=(.*?)"/i', $response->getBody(true), $matches);
            
            if (! $matchesMeta) {
                // match by header
                preg_match('/^\s*(\d*)\s*;\s*url=(.*)/i', (string) $response->getHeader('Refresh'), $matches);
            }
            ...
            if ((! empty($matches)) && (empty($matches[1]) || $matches[1] < $this->refreshMaxInterval)) {
                ...
                $partsCur = parse_url($this->getHistory()
                    ->current()
                    ->getUri());
    ...
    public function getAbsoluteUri($uri)
    {
        ...
        if (strpos($uri, '://') === false) {
            ...
            if (! $this->getHistory()->isEmpty()) {
                return Uri::mergeUrls((string) $this->getHistory()
                    ->current()
                    ->getUri(), $uri);
            }
        }
    ...
    protected function doRequest($request)
    {
        ...
        $guzzleRequest = $this->client->createRequest($request->getMethod(), $request->getUri(), $requestOptions);
