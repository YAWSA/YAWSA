...
class MongoDb
{
    ...
    /**
     * $dsn has to contain db_name after the host.
     * E.g. "mongodb://localhost:27017/mongo_test_db"
     *
     * @static
     *
     * @param
    ...
    public function load($dumpFile)
    {
        if ($this->user && $this->password) {
            $cmd = sprintf('mongo %s --username %s --password %s %s', $this->host . '/' . $this->dbName, $this->user, $this->password, escapeshellarg($dumpFile));
        } else {
