...
class InnerBrowser extends Module implements Web, PageSourceSaver, ElementLocator
{
    ...
    public function _failed(TestCase $test, $fail)
    {
        ...
        $this->_savePageSource(codecept_output_dir() . str_replace([
            '::',
            '\\',
            '/'
        ], [
            ...
            '.',
            '.'
        ], TestCase::getTestSignature($test)) . '.fail.html');
    }
    ...
    {
        return (string) $this->getRunningClient()
            ->getInternalResponse()
            ->getContent();
    }
    ...
    public function click($link, $context = null)
    {
        ...
        if (count($anchor)) {
            $this->crawler = $this->clientClick($anchor->first()
                ->link());
            $this->forms = [];
            return;
        }
        ...
        if (count($button)) {
            ...
            $this->proceedSubmitForm($button->parents()
                ->filter('form')
                ->first(), $buttonValue);
            return;
        }
    ...
    protected function clickByLocator($link)
    {
        ...
        foreach ($nodes as $node) {
            ...
            if ($tag === 'a') {
                $this->crawler = $this->clientClick($nodes->first()
                    ->link());
                $this->forms = [];
                break;
            } elseif (in_array($tag, [
                ...
                $this->proceedSubmitForm($nodes->parents()
                    ->filter('form')
                    ->first(), $buttonValue);
                break;
            }
    ...
    {
        return Uri::retrieveUri($this->getRunningClient()
            ->getHistory()
            ->current()
            ->getUri());
    }
    ...
    {
        $checkboxes = $this->getFieldsByLabelOrCss($checkbox);
        $this->assertEquals(0, $checkboxes->filter('input[checked=checked]')
            ->count());
    }
    ...
    protected function proceedSeeInFormFields($formSelector, array $params, $assertNot)
    {
        ...
        foreach ($params as $name => $values) {
            ...
            if ($field->count() === 0) {
                throw new ElementNotFound(sprintf('//*[@name=%s]', Crawler::xpathLiteral($name)), 'Form');
            }
            if (! is_array($values)) {
                $values = [
                    $values
    ...
    protected function proceedSeeInField(Crawler $fields, $value)
    {
        ...
        return [
            ...
            $value,
            $testValues,
            sprintf('Failed asserting that `%s` is in %s\'s value: %s', $value, $fields->getNode(0)->nodeName, var_export($testValues, true))
        ];
    }
    ...
    protected function setCheckboxBoolValues(Crawler $form, array $params)
    {
        ...
        foreach ($checkboxes as $box) {
            ...
            $skip = (! isset($params[$fieldName])) || (! is_array($params[$fieldName]) && ! is_bool($params[$fieldName])) || ($pos >= count($params[$fieldName]) || (is_array($params[$fieldName]) && ! is_bool($params[$fieldName][$pos])));
            if ($skip) {
                continue;
            }
     ...
     * passed button name.
     *
     * @param Crawler $frmCrawl
     *            the form to submit
     * @param array $params
     *            additional parameter values to set on the
     *            form
     * @param string $button
    ...
    protected function proceedSubmitForm(Crawler $frmCrawl, array $params, $button = null)
    {
        ...
        $this->crawler = $this->clientRequest($form->getMethod(), $url, $requestParams, $form->getPhpFiles());
        $this->forms = [];
    }
    ...
    {
        $currentUrl = $this->getRunningClient()
            ->getHistory()
            ->current()
            ->getUri();
        if (empty($uri) || $uri === '#') {
     ...
     *            the form
     * @param string $action
     *            the form's absolute URL action
     * @return Form
     */
    ...
    public function fillField($field, $value)
    {
        ...
        $dynamicField = $input->getNode(0)->tagName == 'textarea' ? new TextareaFormField($input->getNode(0)) : new InputFormField($input->getNode(0));
    ...
    protected function getFieldsByLabelOrCss($field)
    {
        ...
        // by label
        $label = $this->strictMatch([
            'xpath' => sprintf('.//label[descendant-or-self::node()[text()[normalize-space()=%s]]]', Crawler::xpathLiteral($field))
        ]);
        if (count($label)) {
            $label = $label->first();
            if ($label->attr('for')) {
                $input = $this->strictMatch([
                    'id' => $label->attr('for')
                ]);
            } else {
                $input = $this->strictMatch([
                    'xpath' => sprintf('.//label[descendant-or-self::node()[text()[normalize-space()=%s]]]//input', Crawler::xpathLiteral($field))
                ]);
            }
        }
    ...
    {
        $this->clientRequest($method, $uri, $params, [], [
    ...
    protected function debugResponse($url)
    {
        ...
        $this->debugSection('Request Cookies', $this->getRunningClient()
            ->getInternalRequest()
            ->getCookies());
        $this->debugSection('Response Headers', $this->getRunningClient()
            ->getInternalResponse()
            ->getHeaders());
    }
    ...
    public function grabCookie($cookie, array $params = [])
    {
        $params = array_merge($this->defaultCookieParameters, $params);
        $this->debugCookieJar();
        $cookies = $this->getRunningClient()
            ->getCookieJar()
            ->get($cookie, $params['path'], $params['domain']);
        if (! $cookies) {
            return null;
        }
    ...
    public function seeCookie($cookie, array $params = [])
    {
        $params = array_merge($this->defaultCookieParameters, $params);
        $this->debugCookieJar();
        $this->assertNotNull($this->client->getCookieJar()
            ->get($cookie, $params['path'], $params['domain']));
    }
    ...
    public function seeNumberOfElements($selector, $expected)
    {
        ...
        if (is_array($expected)) {
            ...
            $this->assertTrue($floor <= $counted && $ceil >= $counted, 'Number of elements counted differs from expected range');
        } else {
            $this->assertEquals($expected, $counted, 'Number of elements counted differs from expected number');
        }
    }
    ...
    public function seeInTitle($title)
    {
        ...
        $this->assertContains($title, $nodes->first()
            ->text(), "page title contains $title");
    }
    ...
    public function dontSeeInTitle($title)
    {
        ...
        $this->assertNotContains($title, $nodes->first()
            ->text(), "page title contains $title");
    }
    ...
    protected function assertPageContains($needle, $message = '')
    {
        ...
        $this->assertThat(html_entity_decode(strip_tags($this->_getResponseContent()), ENT_QUOTES), $constraint, $message);
    }

    protected function assertPageNotContains($needle, $message = '')
    {
        $constraint = new PageConstraint($needle, $this->_getCurrentUri());
        $this->assertThatItsNot(html_entity_decode(strip_tags($this->_getResponseContent()), ENT_QUOTES), $constraint, $message);
    }

    protected function assertPageSourceContains($needle, $message = '')
    {
        $constraint = new PageConstraint($needle, $this->_getCurrentUri());
        $this->assertThat($this->_getResponseContent(), $constraint, $message);
    }

    protected function assertPageSourceNotContains($needle, $message = '')
    {
        $constraint = new PageConstraint($needle, $this->_getCurrentUri());
        $this->assertThatItsNot($this->_getResponseContent(), $constraint, $message);
    }
    ...
    public function moveBack($numberOfSteps = 1)
    {
        ...
        try {
            ...
            throw new \InvalidArgumentException(sprintf('numberOfSteps is set to %d, but there are only %d previous steps in the history', $numberOfSteps, $numberOfSteps - $i));
        }
