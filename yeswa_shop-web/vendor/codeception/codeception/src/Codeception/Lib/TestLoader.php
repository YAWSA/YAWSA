...
class TestLoader
{
    ...
    {
        if (! file_exists($path) && substr(strtolower($path), - strlen('.php')) !== '.php' && file_exists($newPath = $path . '.php')) {
            return $newPath;
        }
    ...
    protected function makePath($originalPath)
    {
        ...
        if (file_exists($newPath = $this->findPath($path)) || file_exists($newPath = $this->findPath(getcwd() . "/{$originalPath}"))) {
            $path = $newPath;
        }
