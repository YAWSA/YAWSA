...
EOF;
    ...
    public function produce()
    {
        ...
        return (new Template($this->template))->place('hasNamespace', $namespace ? "namespace $namespace;" : '')
    ...
    protected function prependAbstractActorDocBlocks()
    {
        ...
        foreach ($methods as $method) {
            ...
            $inherited[] = (new Template($this->inheritedMethodTemplate))->place('method', $method->name)
                ->place('params', $params)
    ...
    private function phpEncodeArray(array $array)
    {
        $isPlainArray = function (array $value) {
            return ((count($value) === 0) || ((array_keys($value) === range(0, count($value) - 1)) && (0 === count(array_filter(array_keys($value), 'is_string')))));
        };
