...
class ApnsPHP_Push extends ApnsPHP_Abstract
{
    ...
    public function add(ApnsPHP_Message $message)
    {
        ...
        for ($i = 0; $i < $nRecipients; $i ++) {
            ...
            $this->_aMessageQueue[$nMessageID] = array(
                'MESSAGE' => $message,
                'BINARY_NOTIFICATION' => $this->_getBinaryNotification($message->getRecipient($i), $sMessagePayload, $nMessageID, $message->getExpiry()),
                'ERRORS' => array()
            );
        }
    }
    ...
    {
        if (! $this->_hSocket) {
            throw new ApnsPHP_Push_Exception('Not connected to Push Notification Service');
        }
            ...
            foreach ($this->_aMessageQueue as $k => &$aMessage) {
                ...
                if ($nLen !== ($nWritten = (int) @fwrite($this->_hSocket, $aMessage['BINARY_NOTIFICATION']))) {
                    $aErrorMessage = array(
                        ...
                        'statusMessage' => sprintf('%s (%d bytes written instead of %d bytes)', $this->_aErrorResponseMessages[self::STATUS_CODE_INTERNAL_ERROR], $nWritten, $nLen)
                    );
                }
     ...
     *
     * @param $bEmpty @type
     *            boolean @optional Empty message queue.
     ...
     *
     * @param $bEmpty @type
     *            boolean @optional Empty message container.
     ...
     * @param $nMessageID @type
     *            integer @optional Message unique ID.
     * @param $nExpire @type
     *            integer @optional Seconds, starting from now, that
     ...
     *
     * @param $aErrorMessage @type
     *            array @optional The error message. It will anyway
    ...
    protected function _updateQueue($aErrorMessage = null)
    {
        ...
        $this->_log('ERROR: Unable to send message ID ' . $aErrorMessage['identifier'] . ': ' . $aErrorMessage['statusMessage'] . ' (' . $aErrorMessage['statusCode'] . ').');
     ...
     * @param $bError @type
     *            boolean @optional Insert the message in the Error container.
    ...
    protected function _removeMessageFromQueue($nMessageID, $bError = false)
    {
        if (! is_numeric($nMessageID) || $nMessageID <= 0) {
            throw new ApnsPHP_Push_Exception('Message ID format is not valid.');
        }
        if (! isset($this->_aMessageQueue[$nMessageID])) {
            throw new ApnsPHP_Push_Exception("The Message ID {$nMessageID} does not exists.");
        }
        if ($bError) {
