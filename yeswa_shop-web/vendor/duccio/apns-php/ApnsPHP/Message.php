...
class ApnsPHP_Message
{
     ...
     * Constructor.
     *
     * @param $sDeviceToken @type
     *            string @optional Recipients device token.
     */
     ...
     *
     * @param $sDeviceToken @type
     *            string Recipients device token.
    ...
    public function addRecipient($sDeviceToken)
    {
        if (! preg_match('~^[a-f0-9]{64}$~i', $sDeviceToken)) {
            throw new ApnsPHP_Message_Exception("Invalid device token '{$sDeviceToken}'");
        }
     ...
     * Get a recipient.
     *
     * @param $nRecipient @type
     *            integer @optional Recipient number to return.
    ...
    public function getRecipient($nRecipient = 0)
    {
        if (! isset($this->_aDeviceTokens[$nRecipient])) {
            throw new ApnsPHP_Message_Exception("No recipient at index '{$nRecipient}'");
        }
     ...
     *
     * @param $sText @type
     *            string An alert message to display to the user.
     */
     ...
     *
     * @param $nBadge @type
     *            integer A number to badge the application icon with.
    ...
    {
        if (! is_int($nBadge)) {
            throw new ApnsPHP_Message_Exception("Invalid badge number '{$nBadge}'");
        }
        $this->_nBadge = $nBadge;
    }
     ...
     *
     * @param $sSound @type
     *            string @optional A sound to play ('default sound' is
     *            the default sound).
     */
     ...
     *
     * @param $sCategory @type
     *            string @optional A category for ios8 notification actions.
     */
     ...
     * @see http://tinyurl.com/ApplePushNotificationNewsstand
     *
     * @param $bContentAvailable @type
     *            boolean True to initiates the Newsstand background download.
    ...
    public function setContentAvailable($bContentAvailable = true)
    {
        if (! is_bool($bContentAvailable)) {
            throw new ApnsPHP_Message_Exception("Invalid content-available value '{$bContentAvailable}'");
        }
     ...
     *
     * @param $sName @type
     *            string Custom property name.
     * @param $mValue @type
     *            mixed Custom property value.
    ...
    public function setCustomProperty($sName, $mValue)
    {
        if (trim($sName) == self::APPLE_RESERVED_NAMESPACE) {
            throw new ApnsPHP_Message_Exception("Property name '" . self::APPLE_RESERVED_NAMESPACE . "' can not be used for custom property.");
        }
    ...
    public function getCustomProperty($sName)
    {
        if (! array_key_exists($sName, $this->_aCustomProperties)) {
            throw new ApnsPHP_Message_Exception("No property exists with the specified name '{$sName}'.");
        }
     ...
     *
     * @param $bAutoAdjust @type
     *            boolean If true a long payload is shorted cutting
     *            long text value.
     */
    ...
    /**
     * PHP Magic Method.
     * When an object is "converted" to a string, JSON-encoded
     * payload is returned.
     *
    ...
    public function getPayload()
    {
        ...
        $sJSONPayload = str_replace('"' . self::APPLE_RESERVED_NAMESPACE . '":[]', '"' . self::APPLE_RESERVED_NAMESPACE . '":{}', $sJSON);
        ...
        if ($nJSONPayloadLen > self::PAYLOAD_MAXIMUM_SIZE) {
            if ($this->_bAutoAdjustLongPayload) {
                ...
                if ($nMaxTextLen > 0) {
                    ...
                    throw new ApnsPHP_Message_Exception("JSON Payload is too long: {$nJSONPayloadLen} bytes. Maximum size is " . self::PAYLOAD_MAXIMUM_SIZE . " bytes. The message text can not be auto-adjusted.");
                }
            } else {
                throw new ApnsPHP_Message_Exception("JSON Payload is too long: {$nJSONPayloadLen} bytes. Maximum size is " . self::PAYLOAD_MAXIMUM_SIZE . " bytes");
            }
        }
     ...
     *
     * @param $nExpiryValue @type
     *            integer This message will expire in N seconds
     *            if not successful delivered.
     */
    public function setExpiry($nExpiryValue)
    {
        if (! is_int($nExpiryValue)) {
            throw new ApnsPHP_Message_Exception("Invalid seconds number '{$nExpiryValue}'");
        }
     ...
     *
     * @param $mCustomIdentifier @type
     *            mixed The custom message identifier.
     */
