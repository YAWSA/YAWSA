...
abstract class ApnsPHP_Abstract
{
     ...
     * Constructor.
     *
     * @param $nEnvironment @type
     *            integer Environment.
     * @param $sProviderCertificateFile @type
     *            string Provider certificate file
    ...
    public function __construct($nEnvironment, $sProviderCertificateFile)
    {
        if ($nEnvironment != self::ENVIRONMENT_PRODUCTION && $nEnvironment != self::ENVIRONMENT_SANDBOX) {
            throw new ApnsPHP_Exception("Invalid environment '{$nEnvironment}'");
        }
        $this->_nEnvironment = $nEnvironment;
        
        if (! is_readable($sProviderCertificateFile)) {
            throw new ApnsPHP_Exception("Unable to read certificate file '{$sProviderCertificateFile}'");
        }
     ...
     *
     * @param $logger @type
     *            ApnsPHP_Log_Interface Logger instance.
    ...
    {
        if (! is_object($logger)) {
            throw new ApnsPHP_Exception("The logger should be an instance of 'ApnsPHP_Log_Interface'");
        }
        if (! ($logger instanceof ApnsPHP_Log_Interface)) {
            throw new ApnsPHP_Exception("Unable to use an instance of '" . get_class($logger) . "' as logger: " . "a logger must implements ApnsPHP_Log_Interface.");
        }
        $this->_logger = $logger;
    }
     ...
     *
     * @param $sProviderCertificatePassphrase @type
     *            string Provider Certificate
     *            passphrase.
     */
     ...
     *
     * @param $sRootCertificationAuthorityFile @type
     *            string Root Certification
     *            Authority file.
    ...
    public function setRootCertificationAuthority($sRootCertificationAuthorityFile)
    {
        if (! is_readable($sRootCertificationAuthorityFile)) {
            throw new ApnsPHP_Exception("Unable to read Certificate Authority file '{$sRootCertificationAuthorityFile}'");
        }
     ...
     *
     * @param $nWriteInterval @type
     *            integer Write interval in micro seconds.
     */
     ...
     *
     * @param $nRetryTimes @type
     *            integer Connect retry times.
     */
     ...
     *
     * @param $nRetryInterval @type
     *            integer Connect retry interval in micro seconds.
     */
     ...
     *
     * @param $nSelectTimeout @type
     *            integer Socket select timeout in micro seconds.
     */
    ...
    protected function _connect()
    {
        ...
        $this->_hSocket = @stream_socket_client($sURL, $nError, $sError, $this->_nConnectTimeout, STREAM_CLIENT_CONNECT, $streamContext);
