...
class ApnsPHP_Feedback extends ApnsPHP_Abstract
{
        ...
        while (! feof($this->_hSocket)) {
            ...
            if ($nBufferLen >= $nFeedbackTupleLen) {
                ...
                for ($i = 0; $i < $nFeedbackTuples; $i ++) {
                    ...
                    $this->_log(sprintf("INFO: New feedback tuple: timestamp=%d (%s), tokenLength=%d, deviceToken=%s.", $aFeedback['timestamp'], date('Y-m-d H:i:s', $aFeedback['timestamp']), $aFeedback['tokenLength'], $aFeedback['deviceToken']));
                    unset($aFeedback);
                }
            }
     ...
     *
     * @param $sBinaryTuple @type
     *            string A binary tuple to parse.
