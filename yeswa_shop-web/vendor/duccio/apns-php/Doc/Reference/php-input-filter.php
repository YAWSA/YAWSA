    ...
    while ($mToken = next($aTokens)) {
        ...
        if ($bInParamList) {
            ...
            if (! $bTypedParam && $mToken[TOKEN_TYPE] == T_VARIABLE) {
                if (is_array($aToken[FUNCTION_PARAMETERS]) && isset($aToken[FUNCTION_PARAMETERS][$mToken[TOKEN_STRING]])) {
                    $aTokens[key($aTokens)][TOKEN_STRING] = sprintf('%s %s', $aToken[FUNCTION_PARAMETERS][$mToken[TOKEN_STRING]], $mToken[TOKEN_STRING]);
                }
            }
        }
    }
...
function setFunctionType(&$aTokens)
{
    ...
    while ($mToken = prev($aTokens)) {
        ...
        if (! in_array($mToken[TOKEN_TYPE], array(
            T_DOC_COMMENT,
            T_WHITESPACE,
            T_PRIVATE,
            T_PUBLIC,
